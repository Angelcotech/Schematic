...
| 2026-04-17 | 9.1-9.4 | Symbols. New `extraction/symbols.ts` uses the TS compiler API to extract top-level exports (function/class/interface/type/const) per TS/JS file. Extracted as NodeState entries with kind="symbol" and parent=file_id. Initial attempt rendered symbols on the map inside file boxes; switched to a floating popup after the in-map version looked cluttered and symbols didn't follow file drags. Current design: map shows only modules + files; clicking a file opens a floating panel anchored to its right listing symbols with colored kind-dots and truncated signatures. Popup closes on click-off or selecting another file (driven by `user_state === "selected"` from GraphStore). Layout fix: removed the obsolete MODULE_LABEL_HEIGHT reservation so files center symmetrically in modules (labels live as pills above the box since Stage 8). Module halo switched from proportional pad to fixed 0.18 absolute data units so every module's glow ring reads the same thickness regardless of height. File labels scaled with on-screen box height (no cap) and centered vertically with auto-truncation. Cache SCHEMA_VERSION bumped 1 → 2 so the layout change invalidates old caches. Stage 9b (call-graph between symbols) remains deferred. |
| 2026-04-17 | 10.1-10.7 | CC context integration. `daemon/context/arch-context.ts` builds the `<schematic-context>` block from workspace + selected node + its import/imported-by neighbors (capped at 6). Wired into handleHook — on UserPromptSubmit, daemon returns `{hookSpecificOutput: {additionalContext}}` so CC prepends the block before the prompt reaches Claude. New POST /workspaces/:id/selection endpoint; frontend setSelection() fire-and-forgets to it so the daemon mirrors the browser's clicked node for arch-context. `app/src/mcp/index.ts` — stdio MCP server using `@modelcontextprotocol/sdk`, exposes two tools (`arch_neighbors`, `arch_health`) that proxy to the daemon over HTTP. Tools return a visible "daemon not reachable" message when Schematic is down rather than throwing into the MCP transport — preserves the reference-surface invariant. Install wires `settings.json → "node /.../dist/mcp/index.js"`; echo placeholder from Stage 4 replaced. Deps added: `@modelcontextprotocol/sdk`, `zod`. Verified end-to-end: selecting a file in the browser → UserPromptSubmit hook returns arch-context with its neighbors. |
